...
class JobCassandraDAOSpec extends FunSpecLike with Matchers with BeforeAndAfter with BeforeAndAfterAll {
  private val config = ConfigFactory.load("local.test.jobcassandradao.conf")
  ...
  val jarBytes: Array[Byte] = ByteStreams.toByteArray(getClass.getResourceAsStream("/job-server-tests.jar"))
  var jarFile: File = new File(config.getString("spark.jobserver.cassandradao.rootdir"),
  ...
  private def genJobInfoClosure = {
    ...
    def genTestJobInfo(jarInfo: JarInfo, hasEndTime: Boolean, hasError: Boolean, isNew:Boolean): JobInfo = {
      ...
      val startTime: DateTime = if (isNew) time.plusMinutes(count) else time
  ...
  private def cleanupData() {
    ...
    dao.cqlSession.execute(s"TRUNCATE ${dao.TABLE_JARS}")
    dao.cqlSession.execute(s"TRUNCATE ${dao.TABLE_JAR_ID_LOOKUP}")
    dao.cqlSession.execute(s"TRUNCATE ${dao.TABLE_JOBS}")
    dao.cqlSession.execute(s"TRUNCATE ${dao.TABLE_JOB_CONFIGS}")
  }
  ...
  describe("save and get the jars") {
    it("should be able to save one jar and get it back") {
      ...
      apps(jarInfo.appName).isEqual(jarInfo.uploadTime) should equal(true)
    }
